...
def create_tree(dataset, labels, data_full, labels_full):
    ...
    for value in unique_values:
        ...
        myTree[best_feature_label][value] = create_tree(split_dataset \
                                                      (dataset, best_feature, value), sub_labels, data_full, labels_full)
    if type(dataset[0][best_feature]).__name__ == 'str':
...
def post_pruning_tree(input_tree,dataset,data_test,feature_labels):
    ...
    for key in second_dict.keys():
        if type(second_dict[key]).__name__ == 'dict':
                input_tree[first_feature][key] = post_pruning_tree(second_dict[key],split_dataset(dataset, feature_index, key),split_dataset(data_test, feature_index, key),copy.deepcopy(feature_labels))
...
myTree = create_tree(train_data, feature_labels, train_data_full, feature_labels_full)
print(myTree)
